...
public class SceneCameraController : MonoBehaviour
{
    ...
    private void Rotate()
    {
        ...
        if (Input.GetMouseButton((int)this.rotateTrigger))
        {
            ...
            {
                this.rotateTarget = Quaternion.Euler(0, mouseX * this.rotateSpeed, 0) * this.rotateTarget;  // 这段决定了 axis of x will remain inside worldyz's plane
            }

            if (!this.limitRotateY)
            {
                this.rotateTarget = Quaternion.AngleAxis(mouseY * this.rotateSpeed,
                                                         Vector3.Cross(this.transform.forward, Vector3.up)) * this.rotateTarget;    //决定xy平面的沿axis of worldz的角度
            }
        }
