	...
	void FillAllDrawCalls ()
	{
		...
		for (int i = 0; i < widgets.Count; ++i)
		{
			...
			if (w.isVisible && w.hasVertices)
			{
				...
				if (mat != mt || tex != tx || sdr != sd)	// 如果 渲染设置 调整了，则 step 1. 处理上一批次 to drawCall， 并清空批次指示
				{
					...
					mat = mt;		// step 2. 临时变量指向新的一组 GPU使用数据
					tex = tx;
					sdr = sd;
				}

				if (mat != null || sdr != null || tex != null)	// step 3.
				{
					if (dc == null)					// step 3.1 检测dc是否存在（可能Batch结束过一批dc）
					{
					...
					{
						int rd = w.depth;
						if (rd < dc.depthStart) dc.depthStart = rd;	// 3.2 设置dc的depthStart, depthEnd
						if (rd > dc.depthEnd) dc.depthEnd = rd;
					}

					w.drawCall = dc;		// 

					++count;	//	3.3 geometry.WriteToBuffers, 将widget.geo数据 打入 dc内
		...
		if (dc != null && dc.verts.Count != 0)	// step 4 把留在临时变量中的最后一个dc也add好
		{
			drawCalls.Add(dc);
			dc.UpdateGeometry(count);
			dc.onRender = mOnRender;
